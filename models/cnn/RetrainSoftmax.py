...
tf.app.flags.DEFINE_string('image_dir', '/media/g_fenoglio/D4AF-6366/immaginiTesi/',
                           """Path to folders of labeled images.""")
tf.app.flags.DEFINE_string('output_graph', './retrain1000classesd/output_graph.pb',
                           """Where to save the trained graph.""")
tf.app.flags.DEFINE_string('output_labels', './retrain1000classesd/output_labels.txt',
...
tf.app.flags.DEFINE_string('model_dir', './InceptionNet',
...
def maybe_download_and_extract():
  ...
  if not os.path.exists(dest_directory):
    print('dest_directory not found')
...
def main(_):
  ...
  test_writer = tf.train.SummaryWriter('./log/standard', tf.Session().graph)
  ##INITIALIZE SESSION
  config = tf.ConfigProto(
        device_count = {'GPU': 0}
    )
  sess = tf.Session()
  summary_str = sess.run(tf.histogram_summary('bttProva/primoHist',bottleneck_tensor,tf.GraphKeys.SUMMARIES))
  test_writer.add_summary(summary_str)

  test_writer.flush()

  #print(type(bottleneck_tensor))
