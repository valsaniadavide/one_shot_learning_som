...
def showSomActivations(activations, posActivations, count, title):
    ...
    plt.draw()
    # plt.show()
    img_path = os.path.join(Constants.PLOT_FOLDER, 'temp', 'test.png')
...
def getBMUUPositions(som_path):
    ...
    for c in classes:
        inputs.append(
            getRandomInputClass(c, os.path.join(Constants.DATA_FOLDER, 'input10classes', 'auditoryInput.csv')))
    ...
    for c in inputsU.keys():
        ...
        for u in inputsU[c]:
            ...
            for i in range(len(activations['U'][c][j][0])):
                activations['U'][c][j][0][i] = (
                            float(10.0 * (activations['U'][c][j][0][i] - minA)) / float(maxA - minA))
    ...
    for c in inputsV.keys():
        ...
        for v in inputsV[c]:
            ...
            for i in range(len(activations['V'][c][j][0])):
                activations['V'][c][j][0][i] = (
                            float(10.0 * (activations['V'][c][j][0][i] - minA)) / float(maxA - minA))
...
def testWordLearning():
    ...
    # U = showSom(SOMU,iUall,iUallName,20,'Uditiva')
