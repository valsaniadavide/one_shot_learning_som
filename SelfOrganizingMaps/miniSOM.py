...
class MiniSom(object):
    ...
    def train_random(self, data, num_iteration, verbose=False, compute_qe=False):
        ...
        for iteration in iterations:
            ...
            if (iteration + 1) % 200 == 0 and compute_qe:
                error = self.quantization_error(data)
                self.q_error_pca_init.append(error)
    ...
    def quantization_error_from_neuron(self, data, neuron):
        error = 0
        for x in data:
            error += linalg.norm(x - neuron)
        return error / len(data)
